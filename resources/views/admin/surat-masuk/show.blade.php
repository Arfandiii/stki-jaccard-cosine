...
@section('content')
    <div class="content ml-12 transform ease-in-out duration-500 pt-20 px-2 md:px-5 pb-4">
        ...
        <div class="bg-white px-6 py-4 rounded-t-xl shadow-sm flex justify-between items-center border-b border-gray-200">
            <a href="{{ route('admin.surat-masuk.index') }}" class="text-sm text-blue-600 hover:text-blue-800 font-medium">
        ...
        <div class="bg-gradient-to-r from-blue-500 to-indigo-600 px-6 py-5 shadow-lg">
            <div class="flex items-center space-x-3 text-white">
                ...
                <div>
                    <h2 class="text-2xl font-bold">Detail Surat Masuk</h2>
                    <p class="text-blue-100 text-sm">Informasi lengkap surat yang telah dicatat</p>
                </div>
            </div>
        </div>
                ...
                <div class="space-y-6">
                    ...
                    <div class="border-b pb-4">
                        <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Tanggal
                            Terima</label>
                        <p class="text-lg font-semibold text-gray-800">
                            {{ \Carbon\Carbon::parse($surat->tanggal_terima)->format('d F Y') }}
                        </p>
                    </div>

                    <div>
                        <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wide mb-2">Asal
                            Surat</label>
                        <p class="text-lg font-semibold text-gray-800">{{ $surat->asal_surat }}</p>
                    </div>
                </div>
    ...
    <div x-data="{ open: false, idToDelete: null }" x-on:open-del-modal.window="open = true; idToDelete = $event.detail" x-show="open"
        ...
        <div @click.away="open = false" class="bg-white rounded-lg shadow-xl w-full max-w-sm p-6 mx-4">
            ...
            <div class="flex justify-end space-x-3">
                ...
                <form :action="`{{ url('admin/surat-masuk') }}/${idToDelete}`" method="POST" class="inline">
                    @csrf @method('DELETE')
                    <button type="submit"
